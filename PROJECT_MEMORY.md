...
- **Cloud model**: MiniMax proxy-n át (`ANTHROPIC_BASE_URL=https://api.minimax.io/anthropic`); `ANTHROPIC_DEFAULT_SONNET_MODEL=claude-sonnet-4-6` (Sonnet default, TASK-0010 bevezetve 2026-09-01), `ANTHROPIC_DEFAULT_OPUS_MODEL=claude-opus-4-6` (Opus escalation), `MAIN_AGENT_MODEL=claude-opus-4-6` (főágens), `DEFAULT_AGENT_MODEL=claude-sonnet-4-6` (háttér-jobok, TASK-0012 bevezetve 2026-09-01; service restart TASK-0013-ban élesíti). **FONTOS**: a MiniMax proxy NEM a valódi Claude modelleket szolgáltatja -- a modellnevek (`claude-sonnet-4-6`, `claude-opus-4-6`) a proxy belső `MiniMax-M3` modelljére képződnek le (bizonyíték: 2026-09-01 POST /v1/messages {"model":"claude-sonnet-4-6"} -> response model="MiniMax-M3"). A `.env`-ben a nevek maradnak (a kód ezt olvassa), de a tényleges modell a proxy-n `MiniMax-M3` (lásd `project_minimax_proxy_actual_model.md` memória)
