...
export async function startAgentProcess(name: string, opts: { fresh?: boolean } = {}): Promise<{ ok: boolean; pid?: number; error?: string }> {
  ...
  try {
    ...
    // MiniMax: sajat ag. Enelkul a lenti `isOllama` catch-all magaba szivja
    // (minden nem-claude/deepseek/openrouter modellt Ollamanak feltetelez), es az
    // agens ANTHROPIC_BASE_URL=<ollama>-t kap -> "There's an issue with the
    // selected model (MiniMax-M3)". A base_url-t es a kulcsot a telepites sajat
    // .env-jebol vesszuk (a dashboard process.env-jeben ott van). (2026-08-21)
    const isMiniMax = !isClaude && !isDeepseek && /^minimax/i.test(model)
    ...
    const minimaxBase = process.env['ANTHROPIC_BASE_URL'] ?? ''
    const minimaxKey = process.env['ANTHROPIC_AUTH_TOKEN'] ?? ''
    const minimaxEnv = isMiniMax && minimaxBase && minimaxKey
      ? `export ANTHROPIC_AUTH_TOKEN="${minimaxKey}" && export ANTHROPIC_BASE_URL=${minimaxBase} && export ANTHROPIC_MODEL=${shSingleQuote(model)} && `
      : ''
    ...
    const cmd = `export PATH="/opt/homebrew/bin:$HOME/.bun/bin:/usr/local/bin:/usr/bin:/bin:$PATH" && ${unsetTokens} && ${autoUpdaterEnv}${promptSuggestionEnv}${mcpEnv}${channelSetup}${apiKeyEnv}${claudeConfigEnv}${oauthTokenEnv}${minimaxEnv}${ollamaEnv}${deepseekEnv}${openrouterEnv}cd "${dir}" && ${claudeBin()} ${continueFlag}${skipFlag}--model ${shSingleQuote(model)} ${channelFlag}`.trimEnd()
